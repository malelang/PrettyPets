...
import { Mascota, SimpleResponse, Usuario, Vacuna, Parasito } from '../save-data/save-data';
...
export class NewpetServiceProvider {
  ...
  insertvac(vac: Vacuna[], id: string): Observable<SimpleResponse> {
    const body = { vacunas: vac }
    return this.http.put<SimpleResponse>(this.url1 + "/" + id, body).catch((err) => {
      return Observable.throw(err);
    });
  }

  insertpar(par: Parasito[], id: string): Observable<SimpleResponse> {
    const body = { parasitos: par }
    return this.http.put<SimpleResponse>(this.url1 + "/" + id, body).catch((err) => {
      return Observable.throw(err);
    });
  }
